...
extern volatile uint_fast16_t frame_count;
...
#endif
extern void TCD1_OVF_int(void);
extern void DMA_Led_int(dma_callback_t state);
			...
			case usb_state_set_write:	if(udi_cdc_get_nb_received_data()>=4)
										{
											...
											if((usb_buff[0]==usb_buff[1])&&(usb_buff[2]==usb_buff[3]))
												switch(usb_buff[0])
												{
													...
													case set_gamma:				set.gamma = usb_buff[2]; gamma_calc(&set);	break;
